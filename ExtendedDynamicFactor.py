...
class ExtendedDynamicFactor(MLEModel):
    r"""
    Extended dynamic factor model
    ...
    **kwargs
    ...
    def __init__(self, endog, k_factors, factor_lag, factor_order, exog=None,
        ...
        super(ExtendedDynamicFactor, self).__init__(
    ...
    def _res_classes(self):
        return {'fit': (ExtendedDynamicFactorResults, ExtendedDynamicFactorResultsWrapper)}
    ...
    def start_params(self):
        ...
        if self.k_factors > 0:
            ...
            res_pca = PCA(endog, ncomp=self.k_factors) #standardize=False, demean=False, normalize=False)
    ...
    def _validate_can_fix_params(self, param_names):
        super(ExtendedDynamicFactor, self)._validate_can_fix_params(param_names)
...
class ExtendedDynamicFactorResults(MLEResults):
    """
    Class to hold results from fitting an ExtendedDynamicFactor model.

    Parameters
    ----------
    model : ExtendedDynamicFactor instance
        The fitted model instance

    Attributes
    ----------
    specification : dictionary
        Dictionary including all attributes from the ExtendedDynamicFactor model
        instance.
    ...
    def __init__(self, model, params, filter_results, cov_type=None,
                 **kwargs):
        super(ExtendedDynamicFactorResults, self).__init__(model, params,
    ...
    def summary(self, alpha=.05, start=None, separate_params=True):
        ...
        if spec.k_factors > 0:
            if spec.factor_order > 0:
                model_type = ('ExtendedDynamicFactor(factors=%d, lag=%d, order=%d)' %
...
class ExtendedDynamicFactorResultsWrapper(MLEResultsWrapper):
    _attrs = {}
    ...
    _wrap_methods = wrap.union_dicts(MLEResultsWrapper._wrap_methods,
                                     _methods)
wrap.populate_wrapper(ExtendedDynamicFactorResultsWrapper,  # noqa:E305
                      ExtendedDynamicFactorResults)


class OptimizeExtendedDynamicFactor():
    """
    Optimize the orders of extended dynamic factor model
    ...
    **kwargs
    ...
    ----------
    k_factors : int
        The optimal number of unobserved factors.
    factor_lag : int
        The optimal number of lagged factors in the common component.
    factor_order : int
        The optimal order of the vector autoregression followed by the factors.
    """
    ...
    def _factor_loadings_estimate(self, endog, k_factors):
        # Returns a scaled approximate of the factors and loadings (2nd method of Bai & Ng, 2002)
        T, n = endog.shape
    ...
    def optimize(self, info_criterion=2, info_penalty_weight=1, factor_ic='aic', Dstat=2, delta=0.1, m=1, **kwargs):        
        ...
        Bai & Ng (2007)
        """
        endog = self.endog.copy().dropna().to_numpy()
        T, n = endog.shape
        ...
        return ExtendedDynamicFactor(self.endog, self.k_factors, self.factor_lag, self.factor_order, exog=self.exog,
