...
class STMFSeq2One(nn.Module):
    def __init__(self, dim_x, dim_y, num_layers, n_a=4, n_s=8, n_align=4, fc_y=4, dropout_rate=0, freq_ratio=3, bidirectional_encoder=False):
        ...
        # Decoder
        # self.post_attn = nn.LSTMCell(input_size=n_a + n_a, hidden_size=n_s) # n_a (x) + n_a (y)
    ...
    def forward(self, x_encoder_in, y_decoder_in):
        ...
        # s = torch.zeros(batch_size, self.n_s, device=x_encoder_in.device)
        # c = torch.zeros(batch_size, self.n_s, device=x_encoder_in.device)
        ...
        for t in range(Ty):
            ...
            context = self.one_step_attention(a_to_attend, s[-1])  # Use top layer's hidden state for attention)
            post_attn_input = torch.cat((context, y_a[:, t, :]), dim=-1)
            # s, c = self.post_attn(post_attn_input, (s, c))
            # Pass through stacked LSTMCells
        ...
        # y_pred = F.relu(self.ffn1(s[-1]))  # Use the top layer's hidden state
        y_pred = F.relu(self.ffn1(torch.stack(outputs, dim=1)))  # Stack outputs for all time steps
...
class STMFSeq2OneLightning(pl.LightningModule):
    def __init__(self, dim_x, dim_y, learning_rate, weight_decay, num_layers):
        ...
        self.model = STMFSeq2One(dim_x=dim_x, dim_y=dim_y, num_layers=num_layers)
