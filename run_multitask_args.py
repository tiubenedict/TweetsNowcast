...
# device = 'cpu'
task = "multitask"  # "multitask" or "singletask"
...
for vintage_id in vintage_ids:
    tuner = ray.tune.Tuner(
        ...
        tune_config=ray.tune.TuneConfig(
            ...
            mode="min",
            num_samples=50,
            search_alg=ConcurrencyLimiter(OptunaSearch(metric="val_loss",mode="min"), max_concurrent=8),
        ),
        run_config=ray.tune.RunConfig(
            name=f"{vintage_id.strftime('%Y-%m')}",
            storage_path=f"/home/btiu/Documents/Research/TweetsNowcast/ray_results/{task}_biasTEoptuna4y1x",
            verbose=1,
            checkpoint_config=ray.tune.CheckpointConfig(num_to_keep=1, checkpoint_score_attribute="val_loss", checkpoint_score_order="min"), # Slows down training
        ),
    )
    results = tuner.fit()