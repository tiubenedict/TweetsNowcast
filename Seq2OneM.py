...
class MTSeq2OneM(nn.Module):
    def __init__(self, dim_x, dim_y, num_layers=1, n_a=4, n_s=8, n_align=4, fc_x=4, fc_y=4, dropout_rate=0.0, freq_ratio=3):
        ...
        self.post_attn_cells = nn.ModuleList([
            nn.LSTMCell(input_size=n_a if i == 0 else n_s, hidden_size=n_s)
            for i in range(num_layers)
        ])
        self.decoder_y = Decoder(n_latent=n_s, dim=dim_y, hidden_dim=fc_y)

    def forward(self, x_encoder_in):
        batch_size = x_encoder_in.size(0)
        Lx = x_encoder_in.size(1)
        ...
        outputs = []
        for t in range(Lx//self.freq_ratio):
...
class MTSeq2OneMLightning(pl.LightningModule):
    def __init__(self, dim_x, dim_y,learning_rate, weight_decay, alpha, num_layers):
        super().__init__()
        self.save_hyperparameters()
        self.model = MTSeq2OneM(dim_x=dim_x, dim_y=dim_y, num_layers=num_layers)
    ...
    def forward(self, x_encoder_in):
        return self.model(x_encoder_in)

    def training_step(self, batch, batch_idx):
        x_encoder_in, x_target, y_target = batch
        x_pred, y_pred = self(x_encoder_in)
        ...
        #     print('training',loss.item(), x_encoder_in.shape.shape, x_target.shape, y_target.shape, file=logfile)
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # with open("/home/btiu/Documents/Research/TweetsNowcast/output.log", "a") as logfile:
        #     print('validation',loss_y.item(), loss_x.item(), x_encoder_in.shape.shape, x_target.shape, y_target.shape, file=logfile)
        return loss
