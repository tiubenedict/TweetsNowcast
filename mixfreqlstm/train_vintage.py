...
def train_one_vintage(vintage, device="cpu"):
    ...
    Lx = 27 #trainX_in.shape[1] # 15, 27
    Ty = 8 #trainY_in.shape[1] # 4, 8
    n_a = 4
    n_s = 8
    n_align = 4
    fc_y = 4
    ...
    trainer = pl.Trainer(max_epochs=150, accelerator=device, callbacks=[checkpoint_callback], logger=logger, enable_progress_bar=False, log_every_n_steps=1, deterministic=True)
    tuner = Tuner(trainer)
